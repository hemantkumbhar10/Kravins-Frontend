...
import { Box, Button, Dialog,Slider, SliderValueLabelProps,Stack,Tooltip,Typography,styled, IconButton } from "@mui/material";
...
const AvatarUpload =({close, open}:AvatarUploadProps)=>{
  ...
  const onClickSave = () => {
    ...
    return(
        <Dialog TransitionComponent={Transition} open={open} aria-label='Profile picture upload' sx={{zIndex:999999}}>
          <Box sx={{ width: 'auto',p:2, height: "auto", m:'auto',"& MuiBox-root css-ztrqht":{canvas:{width: {xs:350, sm:500}, touchAction:'none'} }}}>
            <AvatarEditor
      ...
      />
        ...
        <IconButton color="primary" aria-label="rotate picture" onClick={handleRotateRight}>
          <CropRotateIcon sx={{transform:`rotate(-${rotate}deg)`, transition: '10ms ease all'}}/>
        </IconButton>
        </Tooltip>
        </Box>
      ...
      <Typography gutterBottom pb={0} mb={-1}>Zoom</Typography>
      <ImageActionSlider
      ...
      <Button onClick={close} color='blue' variant='contained'>Cancle</Button>
      <Button onClick={onClickSave} color='primary' variant='contained'>Upload</Button>
      </Stack>
